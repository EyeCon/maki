...
data: [DONE]\n\n";
    ...
    #[test]
    fn wire_body_replace_policy_clobbers_whole_computed_value() {
        let compat = compat_with(
            extra_body(&[("repetition_penalty", json!(1.1))]),
            merge_policy(&[("repetition_penalty", ExtraBodyPolicy::Replace)]),
        );
        let merged = compat.wire_body(&json!({"repetition_penalty": {"scale": 2}}));
        assert_eq!(merged["repetition_penalty"], json!(1.1));
    }

    #[test]
    fn wire_body_remove_policy_applies_without_extra_body() {
        let compat = compat_with(
            None,
            merge_policy(&[("stream_options", ExtraBodyPolicy::Remove)]),
        );
    ...
    #[test_case(
        ...
        json!({"a": {"c": 2}}),
        json!({"a": {"b": 1, "c": 2}}) ;
        "objects_merge_recursively"
    )]
    #[test_case(json!([1]), json!([2]), json!([1, 2]) ; "arrays_concatenate")]
    #[test_case(json!({"a": 1}), json!({"a": 5}), json!({"a": 5}) ; "scalar_replaces_scalar")]
    #[test_case(
        json!({"a": {"b": 1}}),
        json!({"a": 5}),
        json!({"a": 5}) ;
    ...
    #[test_case(json!([1]), json!({"x": 1}), json!({"x": 1}) ; "object_replaces_array")]
    #[test_case(
        json!({"a": 1}),
        json!({"a": {"b": 2}}),
        json!({"a": {"b": 2}}) ;
    ...
    fn merge_extra_combines_shapes(computed: Value, extra: Value, expected: Value) {
        let mut merged = computed;
        merge_extra(&mut merged, &extra);
