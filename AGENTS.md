...
- `make check` (or `cargo check -p <crate> --tests`)
- `make lint` - `cargo clippy --all --tests -- -D warnings`
...
Generated by `maki-docgen` (`make gen-docs` / `make gen-docs-check`): tools, providers, configuration, lua-api, plugins, keybindings, commands, folder-trust.
